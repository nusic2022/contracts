    ...
    ERC721Pausable
{
    ...
    function mint(address to_) public onlyWhitelisted returns (uint256) {
        ...
        _mint(to_, _tokenId);
				upateRoyality(_tokenId, 500);
        _tokenIdTracker.increment();
        return _tokenId;
    }

		function mint(address to_, string calldata tokenURI_, uint256 royality_) public onlyWhitelisted {
			uint256 _tokenId = mint(to_);
			updateTokenURI(_tokenId, tokenURI_);
			upateRoyality(_tokenId, royality_);
		}
    ...
    function updateTokenURI(uint256 tokenId_, string calldata uri_) public onlyTokenIdExist(tokenId_) {
        require((ownerOf(tokenId_) == _msgSender() && block.timestamp - _royalityChangeTimestamp[tokenId_] >= royaltyChangeInterval)
            || this.isOperator(_msgSender()), "NusicNFTCore: owner of nft can only set once, operators can set always");
        _tokenURIs[tokenId_] = uri_;
        emit UpdateTokenURI(tokenId_, uri_);
    }

		function upateRoyality(uint256 tokenId_, uint256 rate_) public onlyTokenIdExist(tokenId_) {
        require(
					(ownerOf(tokenId_) == _msgSender() && block.timestamp - _royalityChangeTimestamp[tokenId_] >= royaltyChangeInterval)
            || this.isOperator(_msgSender()), "NusicNFTCore: owner of nft can only set royality after interval from last modification, operators can set always");
